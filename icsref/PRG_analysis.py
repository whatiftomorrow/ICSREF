...
class Program():
    ...
    **Program class**
    ...
    def __find_dynlibs(self):
        ...
        dynlib = re.search(b'[ -~]*', self.hexdump[dynlib_offset:]).group(0).decode('ascii')
        # Find the offsets to dynamic libs
        while dynlib:
            dynlib_offset += len(dynlib) + 1
            temp = self.hexdump[dynlib_offset:dynlib_offset+2].hex()
...
class Function():
    ...
    **Function class**
    ...
    def __init__(self, path, start, stop, hexdump, disasm):
        ...
        self.hash = hashlib.sha256(op_str.encode('ascii')).hexdigest()
